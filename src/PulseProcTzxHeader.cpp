...
int32_t __not_in_flash_func(PulseProcTzxHeader::advance)(
  InputStream *is,
  bool *pstate,
  PulseProc **top
) {
  ...
  DBG_PULSE("About to read TZX header\n");
  int l = is->read(header, 10);

  if (l < 10) {
    DBG_PULSE("Could not read complete TZX header\n");
    return PP_ERROR;
  }
  
  if (strncmp((const char*)header, "ZXTape!", 7) != 0) {
    DBG_PULSE("Could not verify TZX signature bytes\n");
    return PP_ERROR;
  }
  ...
  DBG_PULSE("Found TZX header version %d %d\n", version_major, version_minor);
